        ...
        <section className="rounded-3xl border border-slate-200 bg-white/90 p-7 shadow-lg dark:border-white/10 dark:bg-slate-950/90">
          <header className="space-y-2 border-b border-slate-100 pb-5 dark:border-slate-800">
            ...
            <p className="text-sm md:text-base text-neutral-600 dark:text-neutral-300">
              상품/주문/회사 구성원 관리와 결제까지 이어지는 운영 플로우를 다루는
              서비스입니다. 백엔드에서는 <strong>Product API</strong>를,
              프론트엔드에서는 <strong>회원 관리/주문 관리/프로필</strong> 페이지를
              구현했습니다.
            </p>
          ...
          <div className="mt-5 space-y-4 text-sm md:text-base text-neutral-700 dark:text-neutral-200">
            <section className="space-y-1">
              ...
              <p>
                운영자가 실무에서 바로 쓰는 화면(회원/주문/프로필)과 대용량 목록을
                견디는 상품 API를 만들어, “업무 플로우를 끊김 없이” 이어주는 것이
                목표였습니다.
              </p>
            </section>
            ...
            <section className="space-y-1">
              ...
              <ul className="list-disc space-y-1 pl-5">
                <li>
                  <strong>BE</strong>: Product API(등록/목록·상세/수정/삭제/내 상품),
                  이미지 업로드(S3+CloudFront), 소프트 딜리트 트랜잭션, Swagger 문서화
                </li>
                <li>
                  <strong>FE</strong>: 회원 관리(검색/페이징/초대/삭제/권한), 주문 관리(정렬/페이징/승인·반려/예산 검증),
                  프로필(회사명/비밀번호 변경, 권한 기반 제어), 반응형·ARIA 접근성
                </li>
              </ul>
            </section>
            ...
            <section className="space-y-2">
              ...
              <div className="space-y-3">
                <div className="rounded-2xl border border-slate-200 bg-white/70 p-4 dark:border-slate-700/70 dark:bg-slate-900/50">
                  ...
                  <ul className="mt-2 list-disc space-y-1 pl-5">
                    <li>Controller/Service/Repository 계층 분리 + Swagger 문서화</li>
                    <li>목록 조회: 정렬/카테고리 트리 필터 + 커서 페이지네이션(nextCursor)</li>
                    <li>이미지 업로드: Multer → S3 업로드 → CloudFront URL 반환</li>
                    <li>찜 연동: isFavorite를 서버에서 합성 반환(추가 쿼리 제거)</li>
                    <li>
                      삭제 트랜잭션: Product 소프트 딜리트 + CartItem 소프트 딜리트 +
                      Favorite 정리(원자적 처리)
                    </li>
                  </ul>
                </div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white/70 p-4 dark:border-slate-700/70 dark:bg-slate-900/50">
                  ...
                  <ul className="mt-2 list-disc space-y-1 pl-5">
                    <li>검색/페이징 + 초대/삭제/권한 변경을 단일 화면에서 처리</li>
                    <li>초대/권한수정 모달 통합(검증: Zod + RHF), 충돌 케이스 메시지화</li>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white/70 p-4 dark:border-slate-700/70 dark:bg-slate-900/50">
                  ...
                  <ul className="mt-2 list-disc space-y-1 pl-5">
                    ...
                    <li>승인 플로우에 예산 검증을 통합해 부족 시 결제 진입 차단</li>
                    <li>상태 변경 후 목록/예산 자동 재조회 + 로딩/토스트 피드백</li>
                  </ul>
                </div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white/70 p-4 dark:border-slate-700/70 dark:bg-slate-900/50">
                  ...
                  <ul className="mt-2 list-disc space-y-1 pl-5">
                    ...
                    <li>입력 가드 + “실제 변경” 있을 때만 제출 가능(회사명/비밀번호)</li>
                    <li>성공 시 사용자 재조회 + 토스트 + 리다이렉트(/products)</li>
                  </ul>
                </div>
              </div>
            </section>
